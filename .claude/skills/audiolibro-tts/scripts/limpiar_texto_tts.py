#!/usr/bin/env python3
"""Limpia texto para síntesis de voz en español.

Uso:
    limpiar_texto_tts.py crudo.txt limpio.txt            # texto extraído de PDF
    limpiar_texto_tts.py cap.md limpio.txt --markdown    # capítulo en markdown
"""
...
MD_TITULO = re.compile(r"^\s*#{1,6}\s+(.*)")
MD_REGLA = re.compile(r"^\s*(-{3,}|\*{3,}|_{3,})\s*$")
...
def desmarcar(texto):
    """Quita la sintaxis markdown que la voz leería en alto."""
    fuera = []
    for linea in texto.split("\n"):
        if MD_REGLA.match(linea):
            fuera.append("* * *")                     # regla → cambio de escena
            continue
        m = MD_TITULO.match(linea)
        if m:
            fuera.append("")
            fuera.append(m.group(1).strip() or "")    # el título, sin almohadillas
            fuera.append("")
            continue
        l = re.sub(r"!\[[^\]]*\]\([^\)]*\)", "", linea)      # imágenes
        l = re.sub(r"\[([^\]]+)\]\([^\)]*\)", r"\1", l)      # enlaces → texto
        l = re.sub(r"`{1,3}([^`]*)`{1,3}", r"\1", l)
        l = re.sub(r"(\*\*|__)(.+?)\1", r"\2", l)              # negrita
        l = re.sub(r"(?<![\w*])[*_](?!\s)(.+?)(?<!\s)[*_](?![\w*])", r"\1", l)
        l = re.sub(r"^\s*>\s?", "", l)                          # cita
        fuera.append(l)
    return "\n".join(fuera)

def limpiar(texto, informe, markdown=False):
    if markdown:
    ...
    for pag in paginas:
        ...
        for l in lineas[:2] + lineas[-2:]:
            ...
            if 3 <= len(k) <= 70:      # una cornisa es corta; una frase no
                cuenta[k] += 1
    repetidos = set() if markdown else {k for k, c in cuenta.items()
    ...
    for i, pag in enumerate(paginas):
        ...
        for j, l in enumerate(lineas):
            s = l.strip()
            if not markdown and s and es_numero_pagina(s, i, n, j in bordes):   # 3.1
                fuera_pag += 1
                continue
            if not markdown and j in bordes and clave(s) in repetidos:          # 3.2
                fuera_enc += 1
                continue
            salida.append(l)
        limpias.append("\n".join(salida))
...
if __name__ == "__main__":
    ...
    md = "--markdown" in sys.argv[3:]
    inf = {}
    texto = open(crudo, encoding="utf-8", errors="replace").read()
    limpio = limpiar(texto, inf, markdown=md)
